...
public class TelegramServiceImpl implements TelegramService {
    ...
    private void handleLoggedInCommand(Long chatId, String text, UserSession session) {
        ...
        switch (command) {
            ...
            case "/sprints" -> handleSprints(chatId, parts, session);
            case "/crear_tarea" -> {
            ...
            case "/tareas_sprint" -> handleTareasSprint(chatId, parts, session);
            case "/tareas_usuario" -> handleTareasUsuario(chatId, parts, session);
            case "/asignar_tarea" -> handleAsignarTarea(chatId, parts, session);
            case "/estado_tarea" -> handleEstadoTarea(chatId, parts, session);
    ...
    private void handleSprints(Long chatId, String[] parts, UserSession session) {
        if (parts.length < 2) {
    ...
    private void handleTareasSprint(Long chatId, String[] parts, UserSession session) {
        if (parts.length < 2) {
    ...
    private void handleTareasUsuario(Long chatId, String[] parts, UserSession session) {
        if (parts.length < 2) {
    ...
    private void handleAsignarTarea(Long chatId, String[] parts, UserSession session) {
        if (parts.length < 3) {
    ...
    private void handleEstadoTarea(Long chatId, String[] parts, UserSession session) {
        if (parts.length < 3) {
